 ...
 *   etc.
 * - #orientation: Determines whether the slider handles move horizontally (min on left, max on right)
 *   or vertically (min on bottom, max on top).
 ...
 * - #display_inputs: If set to FALSE will display inputs only when javascript is disabled.
 * - #display_values: If enabled display the current values of slider as simple text.
 * - #display_values_format: Format of the displayed values
 *   The usage is mostly for showing $,% or other signs near the value.
 * - #display_bubble: Display a hint/bubble near each slider handle showing the value of that handle.
 ...
 *   increasing value of one slider decreases the rest of the sliders in the same group.
 ...
 *   values : TRUE , FALSE
 * - #fields_to_sync_css_selector: In case there are other fields that should be sync dynamically when
 *   the slider changes
 *   value : .my_field_css_class
 * - #display_ignore_buton: When field is not required, and display_inputs option is inactive
 *   a checkbox will be shown allowing user to ignore the field
 *   and enter no value values : TRUE , FALSE.
 * - #hide_slider_handle_when_no_value: When the slider does not have any value by enabling this option it won't show the
...
class Slider extends FormElement {
   ...
   */
  public static function preRenderSlider($element) {
    $element['#tree'] = TRUE;
   ...
   *   Form state.
   */
  public function sliderValidatePositiveNumber($element, FormStateInterface $form_state) {
